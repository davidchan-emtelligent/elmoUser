...
def test(options, ckpt_file, data, batch_size=256):
    ...
    with tf.Session(config=config) as sess:
        ...
        for batch_no, batch in enumerate(
            ...
            print("\rbatch=%s  batch_perplexity=%.2f  avg_perplexity=%.2f  time=%.0f      " %
                (batch_no, batch_perplexity, avg_perplexity, time.time() - t1), end=" ")

    avg_loss = np.mean(batch_losses)
    print("\nFINSIHED!  AVERAGE PERPLEXITY = %s" % np.exp(avg_loss))
